...
### 8. ブラー / シャープ (Blur / Sharpen)
このレイヤーまでの合成結果全体に対して、ぼかし（ブラー）または鮮鋭化（シャープ）を適用する後処理レイヤーです。効果をかけたいレイヤーの上に配置してください。

*   **ブレンドモード / 不透明度**: 効果の合成方法を制御します。不透明度は効果全体の強さとしても働きます。
*   **モード**:
    *   `Blur`（ブラー）: 高品質な分離型ガウシアンブラーで画像を柔らかくぼかします。マットキャップの円は元のサイズを保ち、内側の内容だけがぼけます（輪郭が外側に広がることはありません）。
    *   `Sharpen`（シャープ）: アンシャープマスクによりエッジやディテールを強調します。
*   **半径 (Radius)**: ぼかしの大きさ（シャープ時に内部で使うぼかし半径も兼ねます）。値が大きいほど広範囲にぼけます。
*   **強さ (Amount)**: シャープの強さ（`Sharpen`モードで使用）。`Blur`モードでは不透明度で強さを調整してください。
